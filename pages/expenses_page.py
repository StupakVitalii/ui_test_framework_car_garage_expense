...
class ExpensesPage(BasePage):
    ...
    def check_if_loaded(self):
        return True if self.find_element(self.locator.HEADER).text == 'Fuel expenses' else False

    def click_add_expense_button(self):
        self.find_element(self.locator.ADD_NEW_EXPENSE).click()
        return ExpensesPage(self.driver)

    def add_an_expense(self, **kwargs):
        vehicle = Select(self.find_element(self.locator.VEHICLE))
        vehicle.select_by_index(kwargs['vehicle'])
        time.sleep(1)

        report_date = self.find_element(self.locator.REPORT_DATE)
        report_date.clear()
        report_date.send_keys(kwargs['report_date'])

        mileage = self.find_element(self.locator.MILEAGE)
        new_value = int(mileage.get_attribute('value')) + kwargs['mileage']
        mileage.clear()
        mileage.send_keys(new_value)

        number_of_litters = self.find_element(self.locator.NUMBER_OF_LITTERS)
        number_of_litters.clear()
        number_of_litters.send_keys(kwargs['number_of_litters'])

        total_cost = self.find_element(self.locator.TOTAL_COST)
        total_cost.clear()
    ...
    def the_expense_is_added(self, **kwargs):
        expense_table = self.find_element(self.locator.EXPENSE_TABLE)
