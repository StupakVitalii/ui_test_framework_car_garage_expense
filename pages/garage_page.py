...
class GaragePage(BasePage):
    ...
    def check_if_loaded(self):
        return True if self.find_element(self.locator.HEADER).text == 'Garage' else False
    ...
    def add_a_car(self, **kwargs):
        brand = Select(self.find_element(self.locator.BRAND_SELECT))
        brand.select_by_visible_text(kwargs['brand'])
        time.sleep(1)

        model = Select(self.find_element(self.locator.MODEL_SELECT))
        model.select_by_visible_text(kwargs['model'])

        self.find_element(self.locator.MILEAGE_TEXTBOX).send_keys(kwargs['mileage'])
        self.find_element(self.locator.ADD_BUTTON).click()
        return GaragePage(self.driver)

    def the_car_is_added(self, **kwargs):
        added_car = self.find_element(self.locator.CAR_ADDED)
    ...
    def garage_menu(self):
        return self.find_element(self.locator.GARAGE_MENU)

    @property
    def fuel_expenses_menu(self):
        return self.find_element(self.locator.FUEL_EXPENSE_MENU)